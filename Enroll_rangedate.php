...
//    $category = ["과일", "채소", "정육/달걀", "수산물", "유제품", "양곡/견과류", "양념/조미료", "소스", "기타"];
    if($category == "전체"){
        $result = mysqli_query($con, "SELECT * FROM Enroll WHERE user_pid = '".$user_pid."' ORDER BY exp_end asc");
    }
    else{
        $result = mysqli_query($con, "SELECT * FROM Enroll WHERE code_pid = '".$code_pid."' AND category = '".$category."' ORDER BY exp_end asc");
    }
